...
public class DistanceCalc {
    ...
    public double distanceTo(Coordinate source, Coordinate dest, DistanceUnit unit) {
        if (source == null || dest == null) {
            ...
            double x = Math.cos(dest.getLatitude()) * Math.cos(dest.getLongitude()) - Math.cos(source.getLatitude()) * Math.cos(source.getLongitude());//cos(Theta2)*cos(Lamda2)- cos(Theta1)*cos(Lamda1)
            double y = Math.cos(dest.getLatitude()) * Math.sin(dest.getLongitude()) - Math.cos(source.getLatitude()) * Math.sin(source.getLongitude());//cos(Theta2)*sin(Lamda2)- cos(Theta1)*sin(Lamda1)
            double z = Math.sin(dest.getLatitude()) - Math.sin(source.getLatitude());//sin(Theta2)-sin(Theta1)
